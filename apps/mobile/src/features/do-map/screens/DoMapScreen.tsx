...
type Props = NativeStackScreenProps<RootStackParamList, 'DoMap'>

export function DoMapScreen({ navigation }: Props) {
  return (
    <SafeScreen backgroundColor={colors.background} statusBarColor={colors.topbarMid}>
      <View style={styles.container}>
        <PublicTopHeader />
        <PublicWebSection path="/do-map" />
        <PublicBottomMenu current="DoMap" onNavigate={(route) => navigation.navigate(route)} />
      </View>
    </SafeScreen>
  )
}
